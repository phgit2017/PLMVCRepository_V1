...
namespace PL.Infra.Repository
{
    public class IOBalanceV2Repository <T> : IIOBalanceV2Repository<T> where T : class
    {
        protected IIOBalanceDBV2Context Db;

        public IOBalanceV2Repository(IIOBalanceDBV2Context dataContext)
        {
            Db = dataContext;
        }
        ...
        public bool BulkInsert(IList<T> list)
        {
            try
            {
                (Db as System.Data.Entity.DbContext).BulkInsert(list);
                return true;
            }
            catch (Exception)
            {
